...
float GyroMeasError = PI * (40.f / 180.0f);   // gyroscope measurement error in rads/s (start at 40 deg/s)
...
float pitch_initial = 0, yaw_initial = 0, roll_initial = 0;
float pitch_error = 0, yaw_error = 0, roll_error = 0;
float range = 2.0;
...
int CT_maxX = 321, CT_minX = -216, CT_maxY = 551, CT_minY = -75, CT_maxZ = 521, CT_minZ = -84;

bool pushButtonPushed = false, started = false;
int pushButton = 2, speaker = 23;
int rightLed = 21;
int leftLed = 20;
...
void loop()
{
	if(pushButtonPushed && !digitalRead(pushButton))
	{
	...
	if (delt_t > 50) { // update LCD once per half-second independent of read rate
		...
		pitch *= 180.0f / PI;
		yaw   *= 180.0f / PI;
		yaw   -= 1.14f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
		if(yaw < 0) yaw   += 360.0f; // Ensure yaw stays between 0 and 360
		roll  *= 180.0f / PI;
		lin_ax = ax + a31;
		lin_ay = ay + a32;
		...
		yaw_error = yaw - yaw_initial;
		pitch_error = pitch - pitch_initial;
		roll_error = roll - roll_initial;

		if(fabs(yaw_error) > range && started){
			digitalWrite(speaker, HIGH);
			 if((yaw - yaw_initial) > range)
			 {
				digitalWrite(leftLed,LOW);
				digitalWrite(rightLed,HIGH);
			 }
			 else if((yaw - yaw_initial) < (range * -1.0))
			 {
				 digitalWrite(leftLed,HIGH);
				 digitalWrite(rightLed,LOW);
			 }
		...
		if(SerialDebug) {
			...
			Serial.print(roll, 2);
			Serial.print(",");
			Serial.print(yaw_initial, 2);
			Serial.print(",");
			Serial.print(pitch_initial, 2);
			Serial.print(",");
			Serial.print(roll_initial, 2);
			Serial.print(",");
			Serial.print(yaw_error, 2);
			Serial.print(",");
			Serial.print(pitch_error, 2);
			Serial.print(",");
			Serial.print(roll_error, 2);
			Serial.print(",");
