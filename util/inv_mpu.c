        ...
        unsigned char *data)
{
    ...
    if (!data){
    	logString("Error en data");
        return -1;
    }
    if (!st.chip_cfg.sensors){
    	logString("Error en sensors");
        return -1;
    }
    ...
    logValue(tmp[1]);
    logValue(length);
    logValue(st.hw->bank_size);

    /* Check bank boundaries. */
    if (tmp[1] + length > st.hw->bank_size){
    	logString("Error en boundaries");
        return -1;
    }

    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp)){
    	logString("Error en first write");
        return -1;
    }
    if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, data)){
    	logString("Error en second write");
        return -1;
    }
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii])){
        	logString("Error in write");
            return -1;
        }
        if (mpu_read_mem(ii, this_write, cur)){
        	logString("Error in read");
        	return -1;
        }
        if (memcmp(firmware+ii, cur, this_write)){
        	logString("Error in cmp");
        	return -2;
        }
    }
